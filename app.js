...
const tokenSystem = (() => {
    ...
    function addTokens(amount, message = 'Task completed!') {
        const today = isoDate();
        const tokensEarned = parseFloat(amount);
        
        const entry = {
            id: `token-${Date.now()}`,
            date: today,
            savedKg: 0, // Task completion doesn't track specific savings
            tokens: tokensEarned,
            message: message,
            createdAt: Date.now(),
        };
...
function initializeCharts() {
    ...
    try {
        ...
        const dailyBarCtx = document.getElementById('dailyBarChart');
        if (dailyBarCtx) {
            drawLegacyDailyBarChart(dailyBarCtx, api.getCarbonLogs());
        }
        const categoryCtx = document.getElementById('categoryChart');
        if (categoryCtx) {
...
function drawDailyBarChart(canvas) {
    if (!canvas) {
        console.warn('Daily bar chart canvas not found');
        return;
    }
    ...
    if (typeof Chart === 'undefined') {
        drawLegacyDailyBarChart(canvas, logs);
        return;
    }
    ...
    // Ensure we have visible data - if all zeros, add some demo data for visibility
    const hasData = dataPoints.some(point => point.co2e > 0);
    if (!hasData && logs.length === 0) {
        ...
        for (let i = 0; i < days; i++) {
            ...
            dataPoints[i] = {
                ...
                co2e: Math.random() * 0.5 + 0.1 // Random values between 0.1 and 0.6
            };
        }
    }
    ...
    const accentColor = getComputedStyle(document.documentElement).getPropertyValue('--accent-primary').trim() || '#2ad57a';
    const accentSecondary = getComputedStyle(document.documentElement).getPropertyValue('--accent-secondary').trim() || '#5cff9d';

    // Destroy existing chart instance if it exists
    if (window.dailyBarChartInstance) {
        window.dailyBarChartInstance.destroy();
        window.dailyBarChartInstance = null;
    }

    // Create gradient for bars - more vibrant and visible
    const ctx = canvas.getContext('2d');
    const gradient = ctx.createLinearGradient(0, 0, 0, canvas.height || 280);
    gradient.addColorStop(0, accentColor); // Full opacity at top
    gradient.addColorStop(0.5, accentSecondary);
    gradient.addColorStop(1, accentColor); // Full opacity at bottom

    // Create hover gradient (brighter)
    const hoverGradient = ctx.createLinearGradient(0, 0, 0, canvas.height || 280);
    hoverGradient.addColorStop(0, '#5cff9d');
    hoverGradient.addColorStop(1, '#2ad57a');

    window.dailyBarChartInstance = new Chart(canvas, {
        type: 'bar',
        data: {
            labels,
            datasets: [
                {
                    label: 'Daily CO₂e (tonnes)',
                    data: values,
        ...
        options: {
            ...
            scales: {
                y: {
                    beginAtZero: true,
                    min: 0,
                    suggestedMax: Math.max(...values, 0.5) * 1.2 || 1, // Add 20% padding at top
                    grid: {
                        color: 'rgba(154, 216, 178, 0.4)',
                        lineWidth: 1.5,
                        drawBorder: true,
                    },
                    ticks: {
                        callback: (value) => `${Number(value).toFixed(2)} t`,
                        font: {
                            size: 13,
                            weight: '700',
                        },
                        color: '#9ad8b2',
                        padding: 10,
                        stepSize: 0.1, // Show more tick marks
                    },
                    title: {
                        display: true,
                        text: 'CO₂e (tonnes)',
            ...
            plugins: {
                ...
                tooltip: {
                    ...
                    callbacks: {
                        label: (context) => {
                            return `Daily CO₂e: ${context.parsed.y?.toFixed(2) || '0.00'} tonnes`;
                        },
                        title: (context) => {
                            return `Date: ${context[0].label}`;
                        }
                    },
                    backgroundColor: 'rgba(5, 11, 7, 0.98)',
                    titleColor: '#e6ffee',
                    bodyColor: '#9ad8b2',
...
// Legacy Daily Bar Chart (fallback)
function drawLegacyDailyBarChart(canvas, logs) {
    const ctx = canvas.getContext('2d');
    const days = 14;
    const dataPoints = generateDailyData(logs, days);
    const values = dataPoints.map(point => Number((point.co2e || 0).toFixed(2)));
    ...
    const barWidth = (canvas.width - 60) / days;
    const chartHeight = canvas.height - 80;
    const accentColor = '#2ad57a';
    const accentSecondary = '#5cff9d';
    
    // Draw grid lines
    ctx.strokeStyle = 'rgba(154, 216, 178, 0.2)';
    ctx.lineWidth = 1;
    for (let i = 0; i <= 5; i++) {
    ...
    values.forEach((value, index) => {
        ...
        const x = 40 + index * barWidth + barWidth * 0.15;
        ...
        if (value > 0) {
            ...
            ctx.textAlign = 'center';
            ctx.fillText(value.toFixed(2), x + width / 2, y - 5);
        }
    });
    
    // Draw axis labels
    ctx.fillStyle = '#9ad8b2';
...
function generateInsights() {
    ...
    // Use personalized recommendations from features.js if available
    if (typeof window !== 'undefined' && window.hackathonFeatures && window.hackathonFeatures.generatePersonalizedRecommendations) {
        const recommendations = window.hackathonFeatures.generatePersonalizedRecommendations();
        recommendations.forEach(rec => {
            ...
            insightCard.innerHTML = `
                ...
                <p class="insight-text">${rec.description}</p>
                <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 1rem;">
                    <span style="color: var(--accent-primary); font-weight: 600;">${rec.impact}</span>
                    <span style="padding: 0.25rem 0.75rem; background: var(--bg-secondary); border-radius: 12px; font-size: 0.85rem; color: var(--text-secondary);">${rec.difficulty}</span>
                </div>
                <a href="#calculator" class="insight-action">Take Action →</a>
            `;
...
// Goals Setup
function setupGoals() {
    // Tasks are now handled by setupTasks()
    setupTasks();
}
...
function switchProfileTab(tabName) {
    ...
    // Update stats when account tab is opened
    if (tabName === 'account') {
        updateAccountStats();
    }
}
...
async function loadProfile() {
    try {
        ...
        if (user) {
            ...
            if (householdSize) householdSize.value = user.householdSize || 1;
            if (location) location.value = user.location || '';
            if (units) units.value = user.units || 'imperial';
...
function resetProfileForm() {
    if (confirm('Reset all changes? This will reload your saved profile data.')) {
        loadProfile();
    }
}
...
function loadPreferences() {
    ...
    if (goalReminders) goalReminders.checked = preferences.goalReminders !== false;
    if (achievementAlerts) achievementAlerts.checked = preferences.achievementAlerts !== false;
    if (weeklySummary) weeklySummary.checked = preferences.weeklySummary === true;
    if (language) language.value = preferences.language || 'en';
    if (dataSharing) dataSharing.checked = preferences.dataSharing === true;
    if (publicProfile) publicProfile.checked = preferences.publicProfile === true;
}
...
function savePreferences() {
    const preferences = {
        ...
        achievementAlerts: document.getElementById('achievement-alerts')?.checked || false,
        weeklySummary: document.getElementById('weekly-summary')?.checked || false,
...
function changePassword() {
    ...
    showNotification('Password updated successfully! (Demo mode)', 'success');
...
function exportData() {
    ...
    link.href = url;
    link.download = `carbon-tracker-data-${new Date().toISOString().split('T')[0]}.json`;
    link.click();
...
function updateProfileStats() {
    ...
    logs.forEach(log => {
        if (log.timestamp || log.date) {
            const date = (log.timestamp || log.date).split('T')[0];
    ...
    // Update header stats
    const profileTotalCo2 = document.getElementById('profileTotalCo2');
    ...
    if (profileTotalCo2) profileTotalCo2.textContent = totalCo2e.toFixed(1);
    ...
    if (user.householdSize) completion += 25;
...
function updateAccountStats() {
    ...
    const treesEquivalent = Math.ceil(totalCo2e * 20);
    
    // Get goals completed (from localStorage or calculate)
    const goalsCompleted = parseInt(localStorage.getItem('goalsCompleted') || '0');
    
    // Update stats
    const accountTotalCo2 = document.getElementById('accountTotalCo2');
    const accountDaysActive = document.getElementById('accountDaysActive');
    const accountGoalsCompleted = document.getElementById('accountGoalsCompleted');
    const accountTreesEquivalent = document.getElementById('accountTreesEquivalent');
    
    if (accountTotalCo2) accountTotalCo2.textContent = totalCo2e.toFixed(2);
    if (accountDaysActive) accountDaysActive.textContent = daysActive;
    if (accountGoalsCompleted) accountGoalsCompleted.textContent = goalsCompleted;
    if (accountTreesEquivalent) accountTreesEquivalent.textContent = treesEquivalent.toLocaleString();
...
function updateActivityTimeline() {
    ...
    activities.push({
        ...
        description: 'You joined and started tracking your carbon footprint',
        date: new Date(joinDate).toLocaleDateString()
    });
    ...
    if (logs.length > 0) {
        ...
        activities.push({
            ...
            description: `You calculated your first carbon footprint: ${parseFloat(firstLog.co2e || 0).toFixed(2)} tonnes CO₂e`,
            date: firstLog.timestamp ? new Date(firstLog.timestamp).toLocaleDateString() : 'Recently'
        });
    }
    ...
    if (logs.length >= 10) {
        activities.push({
            icon: '🔥',
            title: '10 Calculations Milestone',
            description: 'You\'ve logged 10 carbon footprint calculations!',
            date: 'Recently'
        });
    }
...
// Task System - One task at a time with timing and token rewards
const taskSystem = {
    tasks: [
        {
            ...
            description: 'Choose one day per week to avoid using your car. Walk, cycle, or use public transport instead. This simple change can significantly reduce your carbon footprint!',
            duration: 1, // weeks
        ...
        },
        {
            ...
            description: 'Share rides with others going to the same destination. Reduces emissions per person significantly and saves money too!',
            duration: 2, // weeks
        ...
        },
        {
            ...
            category: 'Energy',
            title: 'Wash clothes in cold water and line-dry',
        ...
        },
        {
            id: 'task_food_local',
            category: 'Food',
            title: 'Buy local fruits & vegetables',
    ...
    getRandomTask() {
        ...
        const availableTasks = this.tasks.filter(task => !completedTaskIds.includes(task.id));
        
        if (availableTasks.length === 0) {
            // All tasks completed, reset or show message
            return null;
        }
    ...
    getActiveTask() {
        ...
        return activeTask ? JSON.parse(activeTask) : null;
    },
    ...
    setActiveTask(task) {
        const taskData = {
            ...
            deadline: new Date(Date.now() + task.duration * 7 * 24 * 60 * 60 * 1000).toISOString(),
            status: 'active'
        };
        localStorage.setItem('activeTask', JSON.stringify(taskData));
        return taskData;
    },
    ...
    completeTask() {
        ...
        // Award tokens
        if (typeof tokenSystem !== 'undefined' && tokenSystem.addTokens) {
            tokenSystem.addTokens(activeTask.tokenReward, `Completed challenge: ${activeTask.title}`);
        }
    ...
    getCompletedTaskIds() {
        return this.getCompletedTasks().map(t => t.id);
    },
    
    calculateProgress() {
        const activeTask = this.getActiveTask();
        if (!activeTask) return { percent: 0, daysRemaining: 0 };
        
        const now = new Date();
        const start = new Date(activeTask.startDate);
        const deadline = new Date(activeTask.deadline);
        const totalDays = activeTask.duration * 7;
        ...
        return { percent, daysRemaining, elapsedDays, totalDays };
    }
};
...
function getNewTask() {
    ...
    if (activeTask) {
        if (!confirm('You already have an active task. Do you want to skip it and get a new one?')) {
            return;
        }
        localStorage.removeItem('activeTask');
    }
    
    const task = taskSystem.getRandomTask();
    if (!task) {
        showNotification('🎉 Congratulations! You\'ve completed all available tasks!', 'success');
        return;
    }
...
function displayTask(task) {
    ...
    const noTaskState = document.getElementById('noTaskState');
    
    if (!activeTaskContainer || !noTaskState) return;
    
    // Hide no task state, show active task
    noTaskState.style.display = 'none';
    activeTaskContainer.style.display = 'block';
    
    // Populate task details
    document.getElementById('activeTaskCategory').textContent = task.category;
    document.getElementById('activeTaskTitle').textContent = task.title;
    document.getElementById('activeTaskDescription').textContent = task.description;
    document.getElementById('activeTaskDuration').textContent = `${task.duration} ${task.duration === 1 ? 'week' : 'weeks'}`;
    document.getElementById('activeTaskSavings').textContent = `${task.estimated_savings_kg} kg CO₂e`;
    document.getElementById('activeTaskTokens').textContent = `${task.tokenReward} tokens`;
    
    // Show start button, hide complete button
    document.getElementById('startTaskBtn').style.display = 'block';
    document.getElementById('completeTaskBtn').style.display = 'none';
    document.querySelector('.active-task-actions button[onclick="skipTask()"]').style.display = 'none';
    
    // Clear progress
    document.getElementById('taskProgressPercent').textContent = '0%';
    document.getElementById('taskProgressFill').style.width = '0%';
    document.getElementById('taskDaysRemaining').textContent = '';
    document.getElementById('activeTaskDeadline').textContent = 'Not started';
}

// Start Active Task
function startActiveTask() {
    // Get the task that's currently displayed
    const taskTitle = document.getElementById('activeTaskTitle')?.textContent;
    if (!taskTitle) {
        showNotification('No task to start', 'error');
        return;
    }
    
    // Find the task in the task system
    const task = taskSystem.tasks.find(t => t.title === taskTitle);
    if (!task) {
        showNotification('Task not found', 'error');
        return;
    }
    
    const activeTask = taskSystem.setActiveTask(task);
    
    // Update UI
    document.getElementById('activeTaskDeadline').textContent = new Date(activeTask.deadline).toLocaleDateString();
    document.getElementById('startTaskBtn').style.display = 'none';
    document.getElementById('completeTaskBtn').style.display = 'block';
    document.querySelector('.active-task-actions button[onclick="skipTask()"]').style.display = 'block';
    
    showNotification('Challenge started! Good luck! 🌱', 'success');
    
    // Start progress tracking
    updateTaskProgress();
    setInterval(updateTaskProgress, 60000); // Update every minute
}
...
function updateTaskProgress() {
    const activeTask = taskSystem.getActiveTask();
    if (!activeTask) {
        // Check if we should load active task
    ...
    const progress = taskSystem.calculateProgress();
    ...
    const daysRemaining = document.getElementById('taskDaysRemaining');
    ...
    if (daysRemaining) {
        if (progress.daysRemaining === 0) {
            daysRemaining.textContent = '⏰ Deadline reached! Complete the task now.';
            daysRemaining.className = 'task-days-remaining danger';
        } else if (progress.daysRemaining <= 2) {
            daysRemaining.textContent = `⚠️ Only ${progress.daysRemaining} day${progress.daysRemaining === 1 ? '' : 's'} remaining!`;
            daysRemaining.className = 'task-days-remaining danger';
        } else if (progress.daysRemaining <= 4) {
            daysRemaining.textContent = `⏳ ${progress.daysRemaining} days remaining`;
            daysRemaining.className = 'task-days-remaining warning';
        } else {
            daysRemaining.textContent = `📅 ${progress.daysRemaining} days remaining`;
...
function completeActiveTask() {
    ...
    if (!confirm(`Complete "${activeTask.title}"?\n\nYou will earn ${activeTask.tokenReward} tokens!`)) {
        return;
    }
    
    const completedTask = taskSystem.completeTask();
    
    // Show success message
    showNotification(`🎉 Challenge completed! You earned ${completedTask.tokenReward} tokens!`, 'success');
    
    // Update token display if available
    if (typeof tokenSystem !== 'undefined' && tokenSystem.updateDisplay) {
        tokenSystem.updateDisplay();
    }
    
    // Hide active task, show no task state
...
// Skip Task
function skipTask() {
    if (!confirm('Skip this challenge? You won\'t earn tokens, but you can get a new task.')) {
        return;
    }
    ...
    document.getElementById('activeTaskContainer').style.display = 'none';
    document.getElementById('noTaskState').style.display = 'block';
    showNotification('Task skipped. Get a new challenge when ready!', 'info');
}
...
function loadActiveTask() {
    ...
    if (activeTask) {
        ...
        document.querySelector('.active-task-actions button[onclick="skipTask()"]').style.display = 'block';
        updateTaskProgress();
        setInterval(updateTaskProgress, 60000);
    } else {
        displayCompletedTasks();
    }
}
...
function displayCompletedTasks() {
    const completedTasks = taskSystem.getCompletedTasks();
    ...
    if (!completedSection || !completedGrid) return;
    ...
    completedGrid.innerHTML = completedTasks.slice(-6).reverse().map(task => `
        <div class="completed-task-card">
            ...
            <div class="completed-task-meta">
                ...
                <span class="completed-task-tokens">+${task.tokenReward} 🪙</span>
            </div>
        </div>
    `).join('');
}
...
// Setup Tasks
function setupTasks() {
    const getNewTaskBtn = document.getElementById('getNewTaskBtn');
    if (getNewTaskBtn) {
        getNewTaskBtn.addEventListener('click', getNewTask);
    }
...
if (typeof window !== 'undefined') {
    ...
    window.startActiveTask = startActiveTask;
    window.completeActiveTask = completeActiveTask;
    window.skipTask = skipTask;
    window.closeActiveTask = closeActiveTask;
}
